        ...
        public Result Connect(string ipAddress, int port)
        {
            ...
            switch (this._Mode)
            {
                ...
                case Mode.ASCII_OVER_TCP_IP:
                    ...
                    try
                    {
                        // 지연 감소 및 빠른 송수신을 위한 설정
                        this.client.NoDelay = true; // Nagle 비활성화
                        if (this.client.Client != null)
                        {
                            this.client.Client.NoDelay = true;
                        }
                        this.client.ReceiveBufferSize = Math.Max(this.client.ReceiveBufferSize, 64 * 1024);
                        this.client.SendBufferSize = Math.Max(this.client.SendBufferSize, 64 * 1024);
                    }
                    catch { }
                    try
                    {
