...
namespace ModBusDevExpress
{
    public partial class Form1 : DevExpress.XtraEditors.XtraForm
    {
        ...
        private int _layoutFeedHeight = 60;   // 원래 높이 유지
        ...
        // 리스타트 상태 표시
        private Label _restartStatusLabel;
        private System.Windows.Forms.Timer _restartStatusTimer;
        ...
        private void InitializeSaveTimer()
        {
            ...
            saveTimer.Interval = _saveInterval * 1000; // 저장주기에 맞춰 설정
        ...
        private void ApplyModernDesign()
        {
            ...
            this.AutoScroll = false;  // 폼 스크롤바 제거
            this.HorizontalScroll.Enabled = false;  // 폼 가로 스크롤 비활성화
            this.VerticalScroll.Enabled = false;    // 폼 세로 스크롤 비활성화

            // 패널 스타일
            panel1.BackColor = bgColor;  // 다크 네이비
            panel1.Dock = DockStyle.Fill; // 데이터 표시 영역과 레이아웃 크기를 일치
            panel1.AutoScroll = false;    // 스크롤바 제거
            panel1.HorizontalScroll.Enabled = false;  // 가로 스크롤 완전 비활성화
            panel1.VerticalScroll.Enabled = false;    // 세로 스크롤 완전 비활성화
            panel1.HorizontalScroll.Visible = false;  // 가로 스크롤바 숨기기
            panel1.VerticalScroll.Visible = false;    // 세로 스크롤바 숨기기
            ...
            button1.Paint += (s, e) =>
            {
                ...
                using (GraphicsPath path = GetRoundedRectangle(rect, 8))
                {
                    ...
                    // 호버 상태에 따른 텍스트 색상만 변경 (배경은 기존 색상 유지)
                    if (button1.ClientRectangle.Contains(button1.PointToClient(Cursor.Position)))
                    {
                        // 마우스 오버 시 텍스트 색상만 변경
                        button1.ForeColor = Color.FromArgb(139, 92, 246); // 보라색으로 변경
                    }
                    else
                    {
                        // 기본 상태
                    ...
                    // 테두리 그리기 (배경색은 기존 Transparent 유지)
        ...
        private void CreateDashboardUI()
        {
            ...
            int feedMargin = 20;  // infoPanel과 동일한 여백
            liveFeedList.Size = new Size(panel1.Width - (feedMargin * 2), feedHeight);
            liveFeedList.Location = new Point(feedMargin, panel1.Height - feedHeight - spacing - infoHeight - spacing);
            ...
            liveFeedList.ScrollAlwaysVisible = false;  // 스크롤바 숨기기
            liveFeedList.HorizontalScrollbar = false;  // 가로 스크롤바 숨기기
            liveFeedList.IntegralHeight = false;       // 높이를 정확히 설정된 값으로 고정
            liveFeedList.DrawMode = DrawMode.Normal;   // 기본 그리기 모드
            ...
            // 3. 하단 정보 영역 (맨 아래) - 높이 보장 (liveFeedList와 정확히 정렬)
            Panel infoPanel = new Panel();
            infoPanel.Name = "infoPanel";
            infoPanel.Size = new Size(panel1.Width - (feedMargin * 2), infoHeight);
            infoPanel.Location = new Point(feedMargin, panel1.Height - infoHeight - spacing);
            ...
            panel1.Resize += (s, e) =>
            {
                ...
                // 하단 영역 재배치 (정보 라인을 항상 맨 아래, 피드는 그 위) - 정확한 정렬 유지
                int resizeFeedMargin = 20;  // 정렬 일관성 유지
                infoPanel.Size = new Size(panel1.Width - (resizeFeedMargin * 2), infoHeight);
                infoPanel.Location = new Point(resizeFeedMargin, panel1.Height - infoHeight - spacing);
                liveFeedList.Size = new Size(panel1.Width - (resizeFeedMargin * 2), feedHeight);
                liveFeedList.Location = new Point(resizeFeedMargin, panel1.Height - feedHeight - spacing - infoHeight - spacing);
                // 상태 아이콘 위치 보정 (설비명 좌측)
                PositionStatusIcon();
                // 같은 라인 유지: 왼쪽/오른쪽 균등 분배 (높이 90 고정) - 간격 균등 조정
                int mh = 90;
                int hs = 15;  // 간격을 15로 통일
        ...
        */
        ...
        private void UpdateDashboardUI(string itemName, double value, string displayValue)
        {
            ...
            if (liveFeedList != null)
            {
                ...
                {
                    // 센터 정렬을 위한 계산
                    string centeredItemName = itemName.PadLeft((20 + itemName.Length) / 2).PadRight(20);
                    string line = $"{DateTime.Now:HH:mm:ss}      | {centeredItemName} | {displayValue,10}";
                    ...
                    if (liveFeedList.Items.Count == 0 || !((string)liveFeedList.Items[liveFeedList.Items.Count - 1]).StartsWith(DateTime.Now.ToString("HH:mm:ss")))
                    {
                        ...
                        // 최대 4개 데이터만 유지 (스크롤바 완전 방지)
                        while (liveFeedList.Items.Count > 4)
                        {
        ...
        public void Log(string msg)
        {
            try
            {
                ...
                using (var sw = new StreamWriter(oFile, append: true, encoding: utf8Bom))
                {
                    sw.WriteLine(logstr);
                }
        ...
        private void LogRestart(string msg)
        {
            try
            {
                string logstr = DateTime.Now.ToString("yyyyMMdd HHmmss") + " " + msg;
                string fileName = $"restart_{DateTime.Now:yyyyMMdd}.log";
                string oFile = Path.Combine(Application.StartupPath, fileName);
                var utf8Bom = new System.Text.UTF8Encoding(true);
                using (var sw = new StreamWriter(oFile, append: true, encoding: utf8Bom))
                {
        ...
        // 간단한 HTML 파서: 재부팅 관련 action/href 추출
        private System.Collections.Generic.IEnumerable<string> DiscoverRebootPathsFromHtml(string html)
        {
            ...
            try
            {
                // href, action에서 reboot/restart/reset/goform/sys/cgi-bin 포함 경로 수집
                string[] keywords = { "reboot", "restart", "reset", "SysTool", "system", "device", "cgi-bin", "goform" };
                var rxHref = new System.Text.RegularExpressions.Regex("(?:href|action)\\s*=\\s*\"([^\"]+)\"", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
                var m = rxHref.Matches(html);
                foreach (System.Text.RegularExpressions.Match mm in m)
                {
                    var url = mm.Groups[1].Value.Trim();
                    if (string.IsNullOrEmpty(url)) continue;
                    if (!url.StartsWith("/")) continue; // 상대 루트 경로만
        ...
        public async void Controller(string ip, string interval, string adrr, string length, string sid, string title = "", string[] aitem = null, string[] aMem = null, string[] aMem2 = null, string saveInterval = "60")
        {
            ...
            Log($"🚀 디바이스 '{lbFaci.Text}' 초기화 완료 - 수집주기: {interval}초, 저장주기: {_saveInterval}초 (개별 저장 타이머 활성화)");
        ...
        // 🏢 회사명 표시 설정
        private void SetupCompanyDisplay()
        {
            try
            {
                var databaseSettings = ConfigManager.LoadDatabaseSettings();
                string selectedCompany = databaseSettings.SelectedCompany;

                if (!string.IsNullOrEmpty(selectedCompany))
                {
                    // 폼 타이틀에 회사명 추가
                    string originalTitle = this.Text;
                    if (!originalTitle.Contains($"[{selectedCompany}]"))
                    {
                        this.Text = $"{originalTitle} [{selectedCompany}]";
                    }

                    Log($"🏢 회사명 표시 설정 완료 - {selectedCompany}");
                }
            }
            catch (Exception ex)
            {
                Log($"❌ 회사명 표시 설정 오류: {ex.Message}");
            }
        }
        ...
        private void UpdateUIWithData(ushort[] data, DateTime timestamp)
        {
            try
            {
                ...
                if (_saveMethod == SaveMethod.Periodic)
                {
                    ...
                    Log($"🔍 _latestData 설정됨 - FacilityCode: '{buffer.FacilityCode}', Values Count: {buffer.Values.Count}, _hasUnsavedData: {_hasUnsavedData}");
                }
                else if (_saveMethod == SaveMethod.ChangeDetection)
                {
                    ...
                    if (hasChanged)
                    {
                        ...
                        Log($"🔍 _latestData 설정됨 (변화 감지) - FacilityCode: '{buffer.FacilityCode}', Values Count: {buffer.Values.Count}, _hasUnsavedData: {_hasUnsavedData}");
                    }
                    else
                    {
        ...
        // 🎯 개별 DB 저장 타이머 이벤트 - 각 디바이스의 저장주기에 맞춰 실행
        private void SaveTimer_Tick(object sender, EventArgs e)
        {
            try
            {
                Log($"🔔 개별 저장 타이머 실행 - {_Faci} (저장주기: {_saveInterval}초)");
                SaveDataToDatabase();
            }
            catch (Exception ex)
            {
                Log($"❌ 개별 저장 타이머 오류 - {_Faci}: {ex.Message}");
            }
        }
        ...
        {
            Log($"🎯 SaveDataToDatabase 호출됨 - 디바이스: {_Faci}");
            Log($"🔍 디버깅 정보 - _hasUnsavedData: {_hasUnsavedData}, _latestData: {(_latestData == null ? "null" : "있음")}");

            if (_latestData != null)
            {
                Log($"🔍 _latestData 상세정보 - FacilityCode: '{_latestData.FacilityCode}', Values Count: {_latestData.Values?.Count ?? 0}, Timestamp: {_latestData.Timestamp:yyyy-MM-dd HH:mm:ss}");
            }

            if (!_hasUnsavedData || _latestData == null)
            {
                Log($"🚫 DB 저장 불가 - _hasUnsavedData: {_hasUnsavedData}, _latestData: {(_latestData == null ? "null" : "있음")}");
                return;
            }

            try
            {
                Log($"💾 DB 저장 시작 - 최신 데이터 (시간: {_latestData.Timestamp:HH:mm:ss})");
                Log($"📋 저장 정보 - 설비: {_latestData.FacilityCode}, IP: {_ip}, 데이터 개수: {_latestData.Values.Count}");
                ...
                foreach (var kvp in _latestData.Values)
                {
                    try
                    {
                        // 선택된 회사명과 저장된 GUID 가져오기
                        var databaseSettings = ConfigManager.LoadDatabaseSettings();
                        string selectedCompany = databaseSettings.SelectedCompany;
                        string selectedCompanyGuidString = databaseSettings.SelectedCompanyGuid;

                        // 저장된 GUID 사용
                        Guid? companyGuid = null;
                        if (!string.IsNullOrEmpty(selectedCompanyGuidString) && Guid.TryParse(selectedCompanyGuidString, out Guid guid))
                        {
                            companyGuid = guid;
                            Log($"🏢 저장된 회사 GUID 사용 - {selectedCompany}: {companyGuid}");
                        }
                        else
                        {
                            Log($"⚠️ 저장된 회사 GUID가 없음 - {selectedCompany}: null로 설정");
                        }

                        var acquiredData = new AcquiredData
                        {
                            CreateUserId = selectedCompany, // 회사명을 CreateUserId 필드에 저장
                            CheckCompanyObjectID = companyGuid, // company 테이블의 실제 GUID
                            CompanyObjectID = companyGuid, // company 테이블의 실제 GUID
                            FacilityCode = _latestData.FacilityCode,
                            NumericData = kvp.Value,
                            StringData = kvp.Key,
                            IPAddress = _ip,
                            CreateDateTime = _latestData.Timestamp
                        };

                        Log($"🔧 객체 생성 완료 - {kvp.Key}: {kvp.Value}");

                        // DbContext에 객체 추가
                        SessionService.Instance.DbContext.ModBusData.Add(acquiredData);
                        savedCount++;

                        Log($"💿 객체 추가 완료 - {kvp.Key} (총 {savedCount}개)");
                    }
                    catch (Exception saveEx)
                    {
                        Log($"❌ 개별 데이터 저장 오류 - {kvp.Key}: {saveEx.Message}");
                        throw;
                    }
                }

                Log($"🔄 SaveChanges 시작 - 저장된 객체 수: {savedCount}");

                // 모든 변경사항을 데이터베이스에 커밋
                SessionService.Instance.SaveChanges();

                Log($"✅ SaveChanges 완료");

                // 저장 확인: 실제 DB에 저장된 데이터 개수 확인
                int totalCount = SessionService.Instance.DbContext.ModBusData.Count();

                var todayStart = DateTime.Now.Date;
                var todayEnd = todayStart.AddDays(1);
                int todayCount = SessionService.Instance.DbContext.ModBusData
                    .Where(x => x.CreateDateTime >= todayStart && x.CreateDateTime < todayEnd)
                    .Count();
                ...
                Log($"✅ DB 저장 완료 - {_Faci} 최신 데이터 (전체: {totalCount}건, 오늘: {todayCount}건)");
            ...
            catch (Exception ex)
            {
                ...
                Log($"❌ 상세 오류: {ex.InnerException?.Message}");
                Log($"❌ 스택 트레이스: {ex.StackTrace}");
            }
        }
        ...
        private async Task AttemptHF2311SRestart()
        {
            ...
            // 방법 1: Others 페이지 Ajax 리스타트 (실제 웹 인터페이스 방식)
            success = await TryOthersPageRestart(deviceIP);
            if (success) return;
        ...
        // Others 페이지 Ajax 리스타트 (실제 웹 인터페이스 방식)
        private async Task<bool> TryOthersPageRestart(string deviceIP)
        {
            try
            {
                Log("🌐 Others 페이지 리스타트 시작");
                ShowRestartStatus($"🎯 리스타트 대상: {deviceIP}", Color.Blue);
                ...
                using (var client = new System.Net.Http.HttpClient())
                {
                    ...
                    // 브라우저처럼 헤더 설정
                    client.DefaultRequestHeaders.UserAgent.ParseAdd("Mozilla/5.0 (Linux; Android 13; SM-G981B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/139.0.0.0 Mobile Safari/537.36");
                    client.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Basic", Convert.ToBase64String(System.Text.Encoding.ASCII.GetBytes("admin:admin")));
                    ...
                    client.DefaultRequestHeaders.Add("Origin", $"http://{deviceIP}");
                    client.DefaultRequestHeaders.Add("Referer", $"http://{deviceIP}/others.html");
                    client.DefaultRequestHeaders.Add("Cookie", "ProductId=EW11; iottitle=EW11");
                    
                    // 1. 메인 페이지 접근 확인
                    Log($"🔍 메인페이지 접근 확인: http://{deviceIP}/");
                    var mainResponse = await client.GetAsync($"http://{deviceIP}/");
                    if (!mainResponse.IsSuccessStatusCode)
                    {
                        Log($"❌ 메인페이지 접근 실패: {mainResponse.StatusCode}");
                        return false;
                    }
                    Log("✅ 메인페이지 접근 성공");
                    ShowRestartStatus("✅ 1단계: 메인페이지 접속 성공", Color.Green);
                    await Task.Delay(1000);
                    
                    // 2. Others 페이지 접근 확인
                    Log($"🔍 Others 페이지 접근 확인: http://{deviceIP}/others.html");
                    var othersResponse = await client.GetAsync($"http://{deviceIP}/others.html");
                    if (!othersResponse.IsSuccessStatusCode)
                    {
                        Log($"❌ Others 페이지 접근 실패: {othersResponse.StatusCode}");
                        return false;
                    }
                    Log("✅ Others 페이지 접근 성공");
                    ShowRestartStatus("✅ 2단계: Others 페이지 접속 성공", Color.Green);
                    await Task.Delay(1000);
                    
                    // 3. 리스타트 Ajax 요청 (dtu.js에서 확인된 정확한 방식)
                    string endpoint = $"http://{deviceIP}/cmd";  // ← /cmd 엔드포인트 사용!
                    Log($"🚀 리스타트 명령 전송: {endpoint}");
                    
                    // Ajax 요청 데이터 (CID:20003, PL:{})
                    var requestData = new
                    {
                        CID = 20003,
                        PL = new { }
                    ...
                    // dtu.js에서 확인된 형식: "msg=" + JSON.stringify(data)
                    string postData = $"msg={jsonData}";
                    ...
                    Log($"📄 응답 내용: {responseText.Substring(0, Math.Min(200, responseText.Length))}...");
                    
                    if (response.IsSuccessStatusCode)
                    {
                        // 성공 응답 확인 (CID:20004, RC:0 예상)
                        if (IsRestartCommandAccepted(responseText))
                        {
                            Log("✅ 리스타트 명령 전달 성공");
                            ShowRestartStatus("✅ 3단계: 리스타트 명령 전달됨", Color.Green);
                            await Task.Delay(2000);
                            
                            // 장치 재시작 확인
                            bool restartVerified = await VerifyDeviceRestart(deviceIP);
                            if (restartVerified)
                            {
                                Log("✅ 장치 재시작 완료 확인");
                                ShowRestartStatus("✅ 7단계: 리스타트 완료!", Color.Green);
                                return true;
                            }
                        }
                        else
                        {
                            Log($"⚠️ 리스타트 명령 응답 이상: {responseText}");
                        }
                    }
                    else
                    {
                        Log($"❌ HTTP 오류: {response.StatusCode} - {responseText}");
                    }
                }
            }
            catch (Exception ex)
            {
                Log($"🚨 Others 페이지 리스타트 실패: {ex.Message}");
            }
        ...
        // 리스타트 명령 수락 여부 확인
        private bool IsRestartCommandAccepted(string responseText)
        {
            try
            {
                // 성공 응답 패턴 확인
                if (responseText.Contains("\"CID\":20004") && responseText.Contains("\"RC\":0"))
                {
                    return true;
                }
                ...
                if (responseText.Contains("success") || responseText.Contains("ok") || 
                    (!responseText.Contains("error") && !responseText.Contains("fail")))
                {
                    return true;
                }
            }
        ...
        // 장치 재시작 확인 (2단계 검증)
        private async Task<bool> VerifyDeviceRestart(string deviceIP)
                    {
                        try
                        {
                Log("🔍 장치 재시작 확인 시작...");
                
                // 1단계: 장치 종료 확인 (Ping 실패)
                Log("🔍 장치 종료 확인중...");
                ShowRestartStatus("🔍 4단계: 장치 종료 확인중...", Color.Orange);
                bool deviceShutdown = await WaitForDeviceShutdown(deviceIP);
                if (!deviceShutdown)
                {
                    Log("⚠️ 장치 종료 확인 실패 - 재시작되지 않았을 수 있음");
                    return false;
                }
                
                // 2단계: 장치 재시작 확인 (Ping 성공 + HTTP 접근)
                Log("🔍 장치 재시작 확인중...");
                ShowRestartStatus("🔍 5단계: 장치 재시작 확인중...", Color.Orange);
                bool deviceRestarted = await WaitForDeviceRestart(deviceIP);
                if (deviceRestarted)
                {
                    Log("✅ 장치 재시작 완료 확인");
                    return true;
            ...
            {
                Log($"🚨 재시작 확인 오류: {ex.Message}");
            return false;
            }
        }

        // 장치 종료 대기 (Ping 실패 확인)
        private async Task<bool> WaitForDeviceShutdown(string deviceIP)
        {
            try
            {
                int timeoutCount = 0;
                ...
                for (int i = 0; i < maxWaitSeconds; i++)
                    {
                        ...
                        var reply = await ping.SendPingAsync(deviceIP, 2000);
                        
                        if (reply.Status != System.Net.NetworkInformation.IPStatus.Success)
                        {
                            timeoutCount++;
                            Log($"📡 Ping 실패 ({timeoutCount}/2): {reply.Status}");
                            
                            if (timeoutCount >= 2)
                            {
                                Log($"✅ 장치 종료 확인 완료 ({i + 1}초 소요)");
                                ShowRestartStatus("✅ 4단계: 장치 종료 확인됨", Color.Green);
                                    return true;
                                }
                            }
                        else
                        {
                            timeoutCount = 0; // 성공하면 카운트 리셋
                            Log($"📡 Ping 성공: {reply.RoundtripTime}ms - 아직 종료되지 않음");
                }
            }
            catch (Exception ex)
            {
                        timeoutCount++;
                        Log($"📡 Ping 예외 ({timeoutCount}/2): {ex.Message}");
                        
                        if (timeoutCount >= 2)
                        {
                            Log($"✅ 장치 종료 확인 완료 (예외, {i + 1}초 소요)");
                            return true;
                        }
                    }
                ...
                Log($"⏰ 장치 종료 대기 시간 초과 ({maxWaitSeconds}초)");
                return false;
            }
            catch (Exception ex)
            {
                Log($"🚨 장치 종료 확인 오류: {ex.Message}");
            return false;
            }
        }

        // 장치 재시작 대기 (Ping 성공 + HTTP 접근 확인)
        private async Task<bool> WaitForDeviceRestart(string deviceIP)
        {
            try
            {
                ...
                for (int i = 0; i < maxWaitSeconds; i++)
                {
                    try
                    {
                        ...
                        if (reply.Status == System.Net.NetworkInformation.IPStatus.Success)
                        {
                            Log($"📡 Ping 성공: {reply.RoundtripTime}ms - 네트워크 복구됨");
                            ...
                            using (var client = new System.Net.Http.HttpClient())
                            {
                                ...
                                // index.html과 others.html 모두 접근 가능한지 확인
                                var indexResponse = await client.GetAsync($"http://{deviceIP}/index.html");
                                var othersResponse = await client.GetAsync($"http://{deviceIP}/others.html");
                                
                                if (indexResponse.IsSuccessStatusCode && othersResponse.IsSuccessStatusCode)
                                {
                                    Log($"✅ 웹 서비스 복구 확인: index.html({indexResponse.StatusCode}), others.html({othersResponse.StatusCode}) - 재시작 완료 ({i + 1}초 소요)");
                                    ShowRestartStatus("✅ 6단계: 웹 서비스 복구 확인됨", Color.Green);
                                    return true;
                                }
                                else
                                {
                                    Log($"⏳ 웹 서비스 아직 준비 안됨: index({indexResponse.StatusCode}), others({othersResponse.StatusCode})");
                        }
                    }
                }
                else
                {
                            Log($"📡 Ping 실패: {reply.Status} - 아직 재시작 중");
                }
            }
            catch (Exception ex)
            {
                        Log($"📡 연결 시도 실패: {ex.Message}");
                    }
                    
                    await Task.Delay(1000);
                }
                
                Log($"⏰ 장치 재시작 대기 시간 초과 ({maxWaitSeconds}초)");
                return false;
            }
            catch (Exception ex)
            {
                Log($"🚨 장치 재시작 확인 오류: {ex.Message}");
                return false;
            }
        }

        // 리스타트 상태 표시 메서드들
        private void ShowRestartStatus(string message, Color color)
            {
                try
                {
                if (InvokeRequired)
                {
                    BeginInvoke(new Action(() => ShowRestartStatus(message, color)));
                    return;
                }

                // 상태 라벨이 없으면 생성
                if (_restartStatusLabel == null)
                {
                    _restartStatusLabel = new Label
                    {
                        AutoSize = true,
                    ...
                    this.Controls.Add(_restartStatusLabel);
                    _restartStatusLabel.BringToFront();
                }

                // 타이머가 없으면 생성
                if (_restartStatusTimer == null)
                {
                    _restartStatusTimer = new System.Windows.Forms.Timer();
                    _restartStatusTimer.Interval = 5000; // 5초 후 자동 숨김
                    _restartStatusTimer.Tick += (s, e) => HideRestartStatus();
                }

                // 상태 표시
                _restartStatusLabel.Text = message;
                _restartStatusLabel.ForeColor = color;
                _restartStatusLabel.Visible = true;

                // 타이머 재시작 (진행 중인 메시지는 자동 숨김 안함)
                _restartStatusTimer.Stop();
                if (!message.Contains("확인중") && !message.Contains("대기"))
                {
                    _restartStatusTimer.Start();
                }
            ...
            {
                Log($"⚠️ 상태 표시 오류: {ex.Message}");
            }
        }

        private void HideRestartStatus()
        {
            try
            {
                if (InvokeRequired)
                {
                    BeginInvoke(new Action(HideRestartStatus));
                    return;
                }

                if (_restartStatusLabel != null)
                {
                    _restartStatusLabel.Visible = false;
                }
